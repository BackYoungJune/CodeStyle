...
namespace Dev_Unit
{					
	public class UnitEnemy_Lamp : UnitEnemy
    {
        //--------------------------------------------------------					
        // 외부 참조 함수 & 프로퍼티					
        //--------------------------------------------------------					
        ...
        {
            base.Awake();
            base.pUniqueID = UnitUniqueID.Enemy_Lamp;
        }
        ...
        {
            base.TurnOver();
            Managers.Battle.DontSkipTurn.Invoke();
        }
        ...
        void AssignAnimatorIDs()
        {
            ...
            base.AnimatorIDTable.AnimID_Hits = new int[2];
        ...
        {
            GameObject projectile = Managers.ObjectPool.GetObjectFromPool(PoolObjectType.Pool, Projectile_Attack); //Instantiate(Projectile_Fireball, genpos.position, genpos.rotation);
            projectile.transform.position = genpos.position;
            projectile.transform.rotation = genpos.rotation;
            projectile.GetComponent<ProjectileSkill>().Initializing(Managers.Unit.GetCurTurnUnit, "Player", Managers.Battle.pGestureReferees);
